/**
 * General MIDI program number (0-127) -> smplr/midi-js-soundfonts instrument
 * slug. Cross-checked against `getSoundfontNames()` from the `smplr` package
 * (the FluidR3_GM / MusyngKite sample sets), including its handful of
 * naming quirks (e.g. GM's "Synth Voice" is shipped as `synth_choir`, "Lead 8
 * (bass+lead)" as `lead_8_bass__lead` with a double underscore).
 */
export const GM_SOUNDFONT_NAMES: string[] = [
...
export function gmSoundfontSlug(program: number): string {
  return GM_SOUNDFONT_NAMES[Math.max(0, Math.min(127, program))] ?? GM_SOUNDFONT_NAMES[0];
}
